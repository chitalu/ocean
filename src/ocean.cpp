...
bool ocean_t::setup(void) {
  ...
  // must be even
  assert((!(obj.length % 2) && !(obj.breadth % 2)) &&
         "grid dimensions not even");
  ...
  glBindVertexArray(obj.vao);
  {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, obj.vbo);
    {
      ...
      glBufferData(GL_ARRAY_BUFFER,
                   sizeof(glm::vec3) * (obj.length * obj.breadth), NULL,
                   GL_DYNAMIC_DRAW);
      ...
      glVertexAttribPointer(obj.vpos_attrib_loc, 3, GL_FLOAT, GL_FALSE, 0,
                            NULL);

      glm::vec3 *gpu_vptr =
          (glm::vec3 *)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
      ...
      for (int x(0); x < obj.length; x++) {
        for (int z(0); z < obj.breadth; z++) {
          gpu_vptr[x + (z * obj.length)] =
              glm::vec3(x - hlength, 0.0f, z - hbreadth);
        }
      }
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, obj.ibo);
    {
      ...
      obj.icount = ((obj.length - 1) * (obj.breadth - 1)) * tris_per_sqr *
                   indices_per_sqr;

      glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * obj.icount, NULL,
                   GL_STATIC_DRAW);
